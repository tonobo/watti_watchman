...
module WattiWatchman
  class Config < Struct.new(:config_data)
    ...
    def self.load(config_path=File.expand_path("../../../watti_watchman.yml", __FILE__))
      main_schema = File.join(SCHEMA_BASE_PATH, "main_schema.yml")

      main_schema_data = YAML.safe_load(File.read(main_schema))
      raw_yaml = YAML.safe_load(File.read(config_path))
      ...
      config_data = JSON.parse(Oj.dump(raw_yaml, mode: :strict))
